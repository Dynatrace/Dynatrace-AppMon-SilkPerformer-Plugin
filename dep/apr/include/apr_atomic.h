...
/**
 * atomically increment an apr_uint32_t
 * @param mem the pointer
 */
APR_DECLARE(void) apr_atomic_inc32_noreturn(volatile apr_uint32_t *mem);

/**
 * atomically decrement an apr_uint32_t
 *@param mem the pointer
 */
APR_DECLARE(void) apr_atomic_dec32_noreturn(volatile apr_uint32_t *mem);

/**
 * test_and_set. atomically check the value of an apr_uint32_t and set it to 1 if
 * the value is 0
 * @param mem the pointer
 * @return the old value pointed to by mem
 */
APR_DECLARE(apr_uint32_t) apr_atomic_tas32(volatile apr_uint32_t *mem);
