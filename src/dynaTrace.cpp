...
BOOL checkIsLoadTest(void) {
	...
	switch (iLoadTestType) {
	case ePI_TestType_LoadTest:
		DLOG("LoadTestType is 'LoadTest'.");
		return TRUE;
	case ePI_TestType_TryScriptTest:
		DLOG("LoadTestType is 'TryScriptTest'.");
		return TRUE;
	default:
		DLOG("LoadTestType not one of [LoadTest,TryScript] - session-recording not started.");
	}
	return FALSE;
}
...
String getDynatraceHeader(String headers) {
	...
	if (!che) {
		Log(eSeverityError, "[getDynatraceHeader] Cannot create buffer for string tokenization!!", 0);
		return retVal;
	} // if
	char * tmp = che;
	char * last = NULL;
...
BOOL startDtClient() {
	...
	if (!CreateProcess(NULL, (LPSTR)cmdLine.getBytes(), NULL, NULL, FALSE, 0, NULL, dir.getBytes(), &si, &pi)) {
		...
		errMsg.sprintf("Cannot start Dynatrace AppMon client: %s", chPszErr);
